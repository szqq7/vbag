...
export interface ProductIndex { products: ProductIndexEntry[]; }
...
let _modules: Record<string, ProductData> | null = null;

export function loadProductIndex(): ProductIndexEntry[] {
  if(!_index)_index=indexData as ProductIndex;return _index?.products||[];
}
export function setProductModules(m: Record<string, ProductData>){_modules=m;}
export function loadProduct(code: string): ProductData|null {
  return _modules?.[`/src/data/products/${code}.json`]||null;
}
...
export function getColorOptions(p: ProductData): ColorOption[] {
  ...
  for(const s of p.productSpecs||[]){
    const n=(s.specsValue1||"").trim();if(!n||seen.has(n))continue;
...
export function getPricingSteps(p: ProductData): PricingStepDisplay[] {
  return(p.productPricingSteps||[]).filter(s=>typeof s.num==="number"&&s.price).sort((a,b)=>a.num-b.num).map(s=>({
    label:`${s.num}+`,quantity:s.num,price:parseFloat(s.price),formatted:`$${parseFloat(s.price).toFixed(2)}`,
  }));
}
...
export function getPrintMethodTabs(p: ProductData):{name:string;pricingSteps:PricingStep[]}[]{
  ...
  const find=(n:string)=>(p.printingWays||[]).find(w=>(w.printingWayNameEn||"").toLowerCase().includes(n.toLowerCase()))
    ?.productPrintingWayPrices?.[0]?.productPrintingWayPriceSteps||base;
  return[{name:"Screen Print",pricingSteps:find("screen")},{name:"Heat Transfer",pricingSteps:find("heat")},{name:"Blank",pricingSteps:base}];
}
...
export function getPrintingSurfaces(p: ProductData):SurfaceDisplay[]{
  return(p.printingSurfaces||[]).map(surface=>({
    ...
    regions:(surface.printingRegions||[]).map(r=>({
      name:r.regionName||surface.printingPositionNameEn||"",width:r.regionWidth||0,height:r.regionHigh||0,
...
export function getSpecifications(p: ProductData):SpecRow[]{
  ...
  return[
    {label:"Standard Color Options",value:colors||"-"},{label:"Material",value:p.material||"-"},
    {label:"Product Size",value:size},{label:"Product Weight",value:wt==="-":"-":`${wt} lbs`},
    {label:"Imprint Method",value:methods},{label:"Imprint Area (inch)",value:area},
    {label:"Packing Method",value:f?.packingMethodEn||"-"},
  ];
}

export function formatPrice(v:number):string{return`$${v.toFixed(2)}`;}
